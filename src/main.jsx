...
const mergeSpeechResults=results=>{let words=[];for(let i=0;i<results.length;i++){const next=results[i][0].transcript.trim().split(/\s+/).filter(Boolean);if(!next.length)continue;const clean=word=>word.toLocaleLowerCase().replace(/[^\p{L}\p{N}']/gu,'');const currentKey=words.map(clean),nextKey=next.map(clean);const prefixLength=Math.min(currentKey.length,nextKey.length);let sharedPrefix=0;while(sharedPrefix<prefixLength&&currentKey[sharedPrefix]===nextKey[sharedPrefix])sharedPrefix++;if(sharedPrefix>=Math.min(3,Math.ceil(prefixLength*.6))){if(next.length>=words.length)words=next;continue}let overlap=Math.min(words.length,next.length);while(overlap>0&&!currentKey.slice(-overlap).every((word,index)=>word===nextKey[index]))overlap--;words=[...words,...next.slice(overlap)]}return words.join(' ')};

function Editable({value,onSave,placeholder,emphasis=false,autoFocus=false,clearOnSave=false,voice=false}){
 const [text,setText]=useState(value||''); const [saving,setSaving]=useState(false); const [listening,setListening]=useState(false); const ref=useRef(null); const savingRef=useRef(false); const recognitionRef=useRef(null); const voiceLimit=useRef(null); const silenceTimer=useRef(null); const tapTimer=useRef(null); const lastTap=useRef(0);
 useEffect(()=>setText(value||''),[value]);
 useEffect(()=>{if(ref.current){ref.current.style.height='auto';ref.current.style.height=`${ref.current.scrollHeight}px`}},[text]);
 useEffect(()=>()=>{clearTimeout(tapTimer.current);clearTimeout(voiceLimit.current);clearTimeout(silenceTimer.current);recognitionRef.current?.abort()},[]);
 ...
 const stopVoice=()=>{clearTimeout(voiceLimit.current);clearTimeout(silenceTimer.current);recognitionRef.current?.stop()};
 const startVoice=()=>{const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SpeechRecognition)return;if(recognitionRef.current)return;const recognition=new SpeechRecognition();const startingText=text.trim();recognitionRef.current=recognition;recognition.continuous=true;recognition.interimResults=true;recognition.lang=navigator.language||'en-US';const resetSilence=()=>{clearTimeout(silenceTimer.current);silenceTimer.current=setTimeout(stopVoice,3000)};recognition.onstart=()=>{setListening(true);resetSilence();voiceLimit.current=setTimeout(stopVoice,10000)};recognition.onresult=event=>{const transcript=mergeSpeechResults(event.results);setText([startingText,transcript].filter(Boolean).join(' '));resetSilence()};recognition.onend=()=>{clearTimeout(voiceLimit.current);clearTimeout(silenceTimer.current);recognitionRef.current=null;setListening(false)};recognition.start()};
 const handleVoiceTap=()=>{if(!voice)return;const now=Date.now();if(now-lastTap.current<350){clearTimeout(tapTimer.current);lastTap.current=0;stopVoice();return}lastTap.current=now;clearTimeout(tapTimer.current);tapTimer.current=setTimeout(()=>{lastTap.current=0;startVoice()},350)};
 return <div className={'editable '+(emphasis?'emphasis ':'')+(listening?'listening':'')}><textarea ref={ref} autoFocus={autoFocus} rows="1" value={text} placeholder={listening?'Listening…':placeholder} onPointerUp={handleVoiceTap} onChange={e=>setText(e.target.value)} onBlur={save} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();e.currentTarget.blur()}}}/>{voice&&<Mic className="inline-voice-icon" size={17}/>} {saving&&<LoaderCircle className="spin" size={15}/>}</div>
}
...
function App(){
 ...
 const [selectedRow,setSelectedRow]=useState(null); const draggingId=useRef(null); const savingOrder=useRef(false); const itemsRef=useRef([]);
  ...
  <section className="board-shell">
   {loading?<div className="loading"><LoaderCircle className="spin"/>Loading your board…</div>:<>
    <div className="board" style={{'--cols':maxNotes+1}}>
     {items.map(item=><div className={`row ${selectedRow===item.id?'selected':''}`} data-row-id={item.id} key={item.id} draggable onClick={event=>handleRowTap(item.id,event)} onDragStart={event=>{draggingId.current=item.id;event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',item.id);event.currentTarget.classList.add('dragging')}} onDragOver={event=>{event.preventDefault();event.dataTransfer.dropEffect='move';moveRow(item.id)}} onDrop={event=>{event.preventDefault();finishRowDrag()}} onDragEnd={event=>{event.currentTarget.classList.remove('dragging');finishRowDrag()}}>
      <div className="cell item-cell"><div className="item-number">{formatTimestamp(item.created_at)}</div><Editable emphasis value={item.title} onSave={v=>updateItem(item.id,v)}/>{item.kanban_notes.length>0&&<button className="next-card" onClick={showNextCard} title="Show next card" aria-label="Show next card"><ArrowRight size={16}/></button>}<button className="delete" onClick={()=>remove(item.id)} title="Delete item"><Trash2 size={15}/></button></div>
      {item.kanban_notes.map((note,index)=><React.Fragment key={note.id}><div className="connector"><ArrowRight size={15}/></div><div className="cell note-cell"><div className="note-meta">{formatTimestamp(note.created_at)}</div><Editable voice value={note.content} onSave={v=>updateNote(note.id,v)}/>{index<item.kanban_notes.length-1&&<button className="next-card" onClick={showNextCard} title="Show next card" aria-label="Show next card"><ArrowRight size={16}/></button>}<button className="delete" onClick={()=>removeNote(note.id)} title="Delete note" aria-label="Delete note"><Trash2 size={15}/></button></div></React.Fragment>)}
      <React.Fragment key={`add-${item.id}-${item.kanban_notes.length}`}><div className="connector"><ArrowRight size={15}/></div><div className="cell add-note"><Editable voice clearOnSave placeholder="Tap to dictate or type an update…" value="" onSave={v=>addNote(item.id,item.kanban_notes.length,v)}/></div></React.Fragment>
     </div>)}
    </div>
